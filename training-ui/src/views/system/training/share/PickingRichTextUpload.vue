    ...
    <el-container>
      <el-form :model="form"
        ...
        <el-main style="padding-top: 5px">
          ...
          <el-form-item label="资讯年份"
                        prop="year">
            <el-select v-model="form.year"
                       placeholder="资讯年份">
              <el-option v-for="dict in yearList"
                         :key="dict.dictValue"
...
import { uploadShare, addShare, updateShare } from '@/api/system/training/share'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        year: [
          { required: true, message: '请选择资讯', trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    wangEdit() {
      ...
      editor.config.placeholder = '请填入培训咨询内容'
      // 后端上传接口
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.uploadData.page === 'edit') {
            ...
            updateShare(JSON.stringify(form)).then((response) => {
            ...
            addShare(JSON.stringify(form)).then((response) => {
              if (response.code === 200) {
