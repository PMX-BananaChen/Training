<template>
  <div class="introduceM">
    ...
    <van-tabs v-model:active="active" @click="getList()" animated>
      ...
      >
        ...
        >
          ...
          <div @click="gotPage(introduces[index])">
            <van-row>
              <van-col span="10">
                <div>
    ...
    <van-empty description="暂无资料" v-if="this.introduces == ''" />
  </div>
</template>
...
import { getIntroduceMobile } from "@/api/system/training/introduce";
import {
  getSynopsis
...
export default {
  ...
  methods: {
    getList() {
      getIntroduceMobile(this.active).then((response) => {
        this.introduces = response.data;
      });
    },
    getDetail() {
      this.queryParams.type = 'introduce';
