<template>
    <div class="inServiceM">
        ...
        <van-image width="100%" height="150px" :src="form.imageName" />
        <van-tabs v-model:active="active" @click="getList()" animated>
            <van-tab v-for="(dict, index) in typeList" :key="index" :title="dict.dictLabel">
                <van-row v-for="(item, index) in inServices" :key="index" style="background-color: #fff" id="rowId">
                    <van-divider :style="{ padding: '0 8px', margin: '6px 0' }" />
                    <div @click="gotPage(inServices[index])">
                        <van-row>
                            <van-col span="10">
                                <div>
        ...
        <van-empty description="暂无资料" v-if="this.inServices == ''" />
    </div>
</template>
...
import { getInServiceMobile } from "@/api/system/training/inService";
import {
    getSynopsis
...
export default {
    ...
    methods: {
        getList() {
            getInServiceMobile(this.active).then((response) => {
                this.inServices = response.data;
            });
        },
        getDetail() {
            this.queryParams.type = 'inService';
