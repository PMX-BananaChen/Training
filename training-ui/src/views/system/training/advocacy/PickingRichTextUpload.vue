      ...
      >
        <el-main style="padding-top: 5px">
          ...
          <el-form-item label="宣导类型" prop="type">
            <el-select v-model="form.type" placeholder="请选择宣导类型">
              <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        title: [
          { required: true, message: "请输入培训宣导标题", trigger: "blur" },
        ],
        type: [{ required: true, message: "请选择宣导", trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    wangEdit() {
      ...
      editor.config.placeholder = "请填写培训宣导内容";
      // 后端上传接口
      ...
      editor.config.customUploadImg = (resultFiles, insertImgFn) => {
        ...
        uploadAdvocacy(formData).then((res) => {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.uploadData.page === "edit") {
            ...
            updateAdvocacy(JSON.stringify(form)).then((response) => {
            ...
            addAdvocacy(JSON.stringify(form)).then((response) => {
              if (response.code === 200) {
