...
import { uploadIntroduce, addIntroduce, updateIntroduce } from '@/api/system/training/introduce'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        title: [
          { required: true, message: '请输入培训介绍标题', trigger: 'blur' }
        ],
        type: [
  ...
  methods: {
    ...
    wangEdit() {
      ...
      editor.config.placeholder = '请填写培训介绍内容'
      // 后端上传接口
      ...
      editor.config.customUploadImg = (resultFiles, insertImgFn) => {
        ...
        uploadIntroduce(formData).then((res) => {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.uploadData.page === 'edit') {
            ...
            updateIntroduce(JSON.stringify(form)).then((response) => {
            ...
            addIntroduce(JSON.stringify(form)).then((response) => {
              if (response.code === 200) {
