  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="介绍类型" prop="type">
        ...
        >
        <el-option
            v-for="dict in dict.type.sys_in_service_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:inService:add']"
          >上传</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:inService:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="2.5">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.sys_in_service_type"
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:inService:edit']"
            >修改</el-button
          >
          <el-button
            size="mini"
...
import {
  ...
  updateInService,
  topInService
} from "@/api/system/training/inService";
import PickingRichTextUpload from "../inService/PickingRichTextUpload";
...
export default {
  ...
  created() {
    this.getList();
    this.getDicts("sys_in_service_type").then((response) => {
  ...
  methods: {
    ...
    getList() {
      ...
      this.queryParams.endTime = this.sertchTime.createTime
        ...
        : null;
      listInService(this.queryParams).then((response) => {
        this.InServiceList = response.rows;
    ...
    handleAdd1() {
      this.reset();
      this.open1 = true;
      this.uploadData1 = { page: "edit",data:"inService"};
      this.title = "在职进修简介设置";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != null) {
            updateInService(this.form).then((response) => {
            ...
            addInService(this.form).then((response) => {
              if (response.code === 200) {
