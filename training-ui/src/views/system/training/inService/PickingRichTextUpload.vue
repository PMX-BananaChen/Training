...
import { uploadInService, addInService, updateInService } from '@/api/system/training/inService'
...
export default {
  ...
  methods: {
    ...
    wangEdit() {
      ...
      editor.config.placeholder = '请填写课件内容'
      // 后端上传接口
      ...
      editor.config.customUploadImg = (resultFiles, insertImgFn) => {
        ...
        uploadInService(formData).then((res) => {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.uploadData.page === 'edit') {
            ...
            updateInService(JSON.stringify(form)).then((response) => {
            ...
            addInService(JSON.stringify(form)).then((response) => {
              if (response.code === 200) {
