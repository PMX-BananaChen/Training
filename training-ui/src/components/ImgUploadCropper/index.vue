<template>
  ...
  import { uploadNEWS} from '@/api/system/training/NEWS'
      ...
      uploadImg() {
        this.$refs.cropper.getCropBlob((data) => {
          ...
          if (this.compType === "avatar") {
            ...
            var fileOfBlob = new File([data],'png');
            formData.append("file", fileOfBlob);
            uploadNEWS(formData).then((response) => {
              if (response.code === 200) {
                this.show = false;
                // this.options.img = process.env.VUE_APP_BASE_API +"/"+response.data[0].imgSrc;
                this.options.img = response.data[0].imgSrc;
                this.$emit("upload-success", response.data[0].imgSrc);
